...
class Handler(webapp2.RequestHandler):
    ...
    # Runs anytime the Handler is called.  Checks for a secure cookie, and if found,
    # verifies the user and returns them
    ...
    # Checks for a cookie containing the users recently viewed blog posts and returns them
...
class Signup(Handler):
    ...
    def post(self):
        ...
        # If all data is valid, hashes the password and sores all of the data in the DB as a User
        if valid_form:
            ...
            # If something does not meet standards, reloads the page and displays the needed error message
    ...
    def post(self):
        ...
        if user:
            ...
            if self.verify_password(password,pw_hash[1]) == pw_hash[0]:
                # If password is verified, cookie is set and user is redirected to the welcome page
...
class NewPost(Handler):
    ...
    def post(self):
        ...
        if self.u:
            ...
            # Checks to see if data is valid. If it is, it creates a new PostObject and stores it in the DB
        ...
        if self.u:
            ...
            if type == 'post':
                ...
                if o.posted_by == self.u.username:
                    # Grabs all comments from this blog post and deletes them and then deletes the PostObject
                ...
                # Grabs the comment that is to be deleted as well as the PostObject it belogs to
            ...
            # Short delay to allow the DB to process the request before the page reloads 
...
class LikePost(Handler):
    def post(self,type,object_id):
        ...
        if self.u:
            o = None
            e = False
            # Checks if user is liking a post or a comment and grabs the objects
            if type == 'post':
            ...
            if self.u.username != o.posted_by:
                # Checks to see if the user has already liked the post. If so, it 'unlikes' it by
                # removing their name from the 'like' list. Otherwise it adds them to the 'like' list
                ...
                o.put()
                self.u.put()
                # Short delay to allow the DB to process the update before the next page loads
                time.sleep(0.3)
        ...
        if self.u:
            ...
            if comment:
                ...
                # Editing an existing comment
                    ...
                    # Gets comment and verifies that its author is the current user
